...
snaive_model<- function(train_ts, test_ts){
  ...
  train_ts <- preprocess.svd(train_ts, 12)
...
harmonic_model<- function(train_ts, test_ts){
  num_forecasts <- nrow(test_ts)
  train_ts[is.na(train_ts)] <- 0
  train_ts <- preprocess.svd(train_ts, 12)
...
linear_model<- function(train_ts, test_ts){
  ...
  for(j in 2:ncol(train_ts)){
    store_ts <- ts(train_ts[, j], frequency=52)
    f_tslm <- tslm(store_ts~trend + season)
    test_ts[, j] <- forecast(f_tslm, num_forecasts)$mean
  }
  test_ts
}
...
mypredict <- function() {
  ...
  for (dept in test_depts) {
    ...
    harmonic <- harmonic_model(train_dept_ts, test_dept_ts)
    test_month <- update_forecast(test_month, harmonic, dept, 2)
    
   # linear <- linear_model(train_dept_ts, test_dept_ts)
    # test_month <- update_forecast(test_month, linear, dept, 3)
    ...
    if(t<7)
    {
      ...
      harmonic <- harmonic_model(train_dept_ts, test_dept_ts)
      test_month <- update_forecast(test_month, harmonic, dept, 3)
    ...
    else
    {
      ...
      stlf <- stlf.svd(train_dept_ts, test_dept_ts, 'arima', n.comp = 12)
      test_month <- update_forecast(test_month, stlf, dept, 3)
    }
